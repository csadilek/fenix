...
class SessionTest {
    ...
    fun `toTab uses active reader URL`() {
        val sessionWithoutReaderState = Session("https://example.com")
        val tabWithoutReaderState = createTab(url = sessionWithoutReaderState.url, id = sessionWithoutReaderState.id)

        val sessionWithInactiveReaderState = Session("https://blog.mozilla.org")
        ...
        val sessionWithActiveReaderState = Session("moz-extension://123")
